    ...
    <div className="relative group w-full">
      <div className={`flex items-center gap-2 p-1.5 rounded-2xl transition-all duration-300 border-2 ${isFocused ? 'bg-white border-blue-500 shadow-xl scale-[1.02]' : 'bg-white/90 backdrop-blur-md border-transparent shadow-lg'}`}>
        ...
        <input 
          ...
          className="w-full py-2 bg-transparent focus:outline-none text-gray-800 font-medium placeholder-gray-400"
          placeholder="Explore the earth..." 
          autoComplete="off"
        />
        {q && (
           <button onClick={() => {setQ(''); setRes([])}} className="p-2 text-gray-300 hover:text-gray-600 transition-colors">
          ...
          {res.map((r, idx) => (
            ...
            >
              ...
              <div className="flex flex-col flex-grow truncate">
                ...
                <span className="text-[10px] text-gray-400 truncate uppercase tracking-wider">{r.display_name.split(',').slice(1).join(',')}</span>
              </div>
            </div>
          ))}
        </div>
